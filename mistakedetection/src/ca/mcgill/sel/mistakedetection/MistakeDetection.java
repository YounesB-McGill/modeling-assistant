...
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.associationEndMultiplicityLowerBoundsMatch;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.associationEndMultiplicityMatch;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.associationEndMultiplicityUpperBoundsMatch;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.attributeTypesMatch;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.createMistake;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isAssociationClassExtra;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isAssociationClassMissing;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isAttributeExpectedStatic;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isAttributeNotExpectedStatic;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isLowerName;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isRoleNameExpectedStatic;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isRoleNameMissing;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isRoleNameNotExpectedStatic;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isUsingAggregationInsteadOfAssociation;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isUsingAggregationInsteadOfComposition;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isUsingAssociationInsteadOfAggregation;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isUsingAssociationInsteadOfComposition;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isUsingCompositionInsteadOfAggregation;
import static ca.mcgill.sel.mistakedetection.MistakeDetectionUtils.isUsingCompositionInsteadOfAssociation;
...
public class MistakeDetection {
  ...
  /** Checks if mapped instructor class is a superclass of a student or vice-versa.*/
    ...
    for (var set : comparison.studentSuperclassesToSubclasses.entrySet()) {
      ...
      for (Classifier subClass : subClasses) {
        ...
        if ((areAttributesEqual(superClassAttributes, subClassAttributes)
            && subClassesAttriAssocEqual(superClass, subClass, comparison) && !superClass.isAbstract())
            || (subClassAttributes.isEmpty() && subClass.getAssociationEnds().isEmpty())) {
  ...
  private static List<Mistake> getAssocInsteadOfComposMistakes(List<Mistake> newMistakes) {
    return newMistakes.stream().filter(m -> m.getMistakeType().equals(USING_ASSOC_INSTEAD_OF_COMPOSITION))
  ...
  private static void checkStudentAOPattern(TagGroup tg, Comparison comparison, Solution studentSolution) {
    ...
    comparison.abstractionOccurrenceAssoc = getAssocAggCompFromClassDiagram((Classifier) instructorElements.get(0),
        (Classifier) instructorElements.get(1), comparison.instructorCdm).get(0);
    if (matchedElements == 0 && !instructorElements.isEmpty()) {
      comparison.newMistakes.add(createMistake(MISSING_AO_PATTERN, null, instructorElements));
    } else if (matchedElements != 0 && instructorElements.size() == matchedElements) {
  ...
  private static boolean containsAssociation(NamedElement abstraction, List<NamedElement> occurrences,
    ...
    for (var occ : occurrences) {
      for (var assoc : comparison.notMappedInstructorAssociations) {
        var cls0 = assoc.getEnds().get(0).getClassifier();
        var cls1 = assoc.getEnds().get(1).getClassifier();
        if ((cls0.equals(occ) && cls1.equals(abstraction)) || (cls1.equals(occ) && cls0.equals(abstraction))) {
          return false;
        }
      }
    }
  ...
  private static AssociationEnd getStudentAssocEnd(AssociationEnd instAssocEnd, Association assoc,
      Comparison comparison) {
  ...
  private static void detectMistakeInAssoc(Comparison comparison, Association studentClassifierAssoc,
    ...
    checkMultiplicityAndRoleNames(studentClassifierAssocEnd, instructorClassifierAssocEnd, comparison);
    checkMultiplicityAndRoleNames(otherStudentClassifierAssocEnd, otherInstructorClassifierAssocEnd, comparison);
  }

  /** Checks the multiplicity and role name for a student association end. */
  private static void checkMultiplicityAndRoleNames(AssociationEnd studentAssocEnd, AssociationEnd instructorAssocEnd,
      Comparison comparison) {
    if (studentAssocEnd.isNavigable() && instructorAssocEnd.isNavigable()) {
      checkMultiplicity(studentAssocEnd, instructorAssocEnd, comparison);
      checkRoleNames(studentAssocEnd, instructorAssocEnd, comparison);
    }
  }
  ...
  public static boolean checkClassAndAttribBasedOnSubStrings(Classifier instructorClass, Classifier studentClass) {
    return studentClass.getName().toLowerCase().contains(instructorClass.getName().toLowerCase())
        || containsSynonym(instructorClass, studentClass);
  }
  ...
  public static Optional<Mistake> checkMistakeMissingRoleName(AssociationEnd studentClassAssocEnd,
    ...
    if (isRoleNameMissing(studentClassAssocEnd, instructorClassAssocEnd)) {
      return Optional.of(createMistake(MISSING_ROLE_NAME, studentClassAssocEnd, instructorClassAssocEnd));
    }
    return Optional.empty();
  }
  ...
  public static void checkMistakeMissingAssociationCompositionAggregation(Comparison comparison) {
    for (Association association : comparison.notMappedInstructorAssociations) {
      ...
      if (association.getAssociationClass() != null
          && !comparison.mappedClassifiers.containsKey(association.getAssociationClass())) {
  ...
  public static void checkMistakeExtraAssociationCompositionAggregation(Comparison comparison) {
    for (Association association : comparison.extraStudentAssociations) {
      ...
      if (association.getAssociationClass() != null
          && !comparison.mappedClassifiers.containsValue(association.getAssociationClass())) {
        removeMistakesRelatedToElement(association.getAssociationClass(), comparison.newMistakes);
        comparison.newMistakes
