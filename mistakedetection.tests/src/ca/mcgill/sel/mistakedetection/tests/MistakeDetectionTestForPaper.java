...
public class MistakeDetectionTestForPaper {
  ...
  public void mistakeDetectionForPaperExample() {
    ...
    for (Mistake m : studentSolution.getMistakes()) {
      assertMistakeInLoop(m, WRONG_ATTRIBUTE_TYPE, studentBadgeNumberAttribute,
          instructorBadgeNumberAttribute, 0, 1, false);
      assertMistakeInLoop(m, PLURAL_CLASS_NAME, studentAssignementsClass,
          instructorAssignementClass, 0, 1, false);
      assertMistakeInLoop(m, PLURAL_CLASS_NAME, studentPeopleClass, instructorPersonClass, 0, 1, false);
      assertMistakeInLoop(m, BAD_ATTRIBUTE_NAME_SPELLING, studentAddresAttribute,
          instructorAddressAttribute, 0, 1, false);
      assertMistakeInLoop(m, OTHER_WRONG_MULTIPLICITY, studentAssocEndOfficers,
          instructorAssocEndAssignedOfficer, 0, 1, false);
      assertMistakeInLoop(m, OTHER_WRONG_ROLE_NAME, studentAssocEndOfficers,
          instructorAssocEndAssignedOfficer, 0, 1, false);
      assertMistakeInLoop(m, OTHER_WRONG_ROLE_NAME, studentAssocEndStations,
          instructorAssocEndPoliceStation, 0, 1, false);
      assertMistakeInLoop(m, USING_AGGREGATION_COMPOSITION_INSTEAD_OF_ASSOCIATION,
          studentAssocEndOfficer, instructorAssocEndOfficer, 0, 1, false);
      assertMistakeInLoop(m, OTHER_WRONG_ROLE_NAME, studentAssocEndWorkAt,
          instructorAsscocEndWorkLocation, 0, 1, false);
      assertMistakeInLoop(m, MISSING_ATTRIBUTE, instructorEndDateAttribute, 0, 1, false);
      assertMistakeInLoop(m, MISSING_COMPOSITION, instructorPoliceOfficer_PISystem, 0, 1, false);
    }
  }
